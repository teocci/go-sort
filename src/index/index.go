...
// equal key.
// It can return an empty range if the item isn't found; in that case, a and b are both
// where the item would be inserted (and can be one past the end).
...
// FindStringRange finds the range (a,b] such that Key() returns key for all items in idx.Data[a:b].
// It can return an empty range if the item isn't found; in that case, a and b are both where the
// item would be inserted (and can be one past the end).
...
// FindBytesRange finds the range (a,b] such that Key() returns key for all items in idx.Data[a:b].
// It can return an empty range if the item isn't found; in that case, a is where the item would
// be inserted (and can be one past the end).
// Data must implement Key(i) returning string or []byte.
...
// any of core.StringInterface, BytesInterface, or Uint64Interface.
